...
class TimerControllers {
  ...
  async listPubsubChannels () {
    try {
      ...
      if (!ipfs || !ipfs.libp2p) {
        this.adapters.log.statusLog(1, 'IPFS/libp2p instance not available')
        return false
      }

      const pubsub = ipfs.libp2p.services.get?.('pubsub') || ipfs.libp2p.services.pubsub
      if (!pubsub) {
        this.adapters.log.statusLog(1, 'Pubsub service not available')
        return false
      }

      const pubsubChans = pubsub.getTopics()
      ...
      console.error('Error listing pubsub channels:', err)
      this.adapters.log.statusLog(1, `Error listing pubsub channels: ${err.message}`)
      return false
    }
  }
