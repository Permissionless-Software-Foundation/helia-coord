  ...
  async addSubnetPeer (announceObj) {
    try {
      ...
      if (!this.thisNode.peerList.includes(thisPeerId)) {
        ...
        try {
          const pubsub = this.adapters.ipfs.ipfs.libp2p.services.get?.('pubsub') ||
                         this.adapters.ipfs.ipfs.libp2p.services.pubsub
          if (!pubsub) {
            throw new Error('Pubsub service not available')
          }
          await pubsub.subscribe(thisPeerId)
          this.adapters.log.statusLog(2, `Subscribed to peer pubsub channel ${thisPeerId}`)
        } catch (err) {
          console.error(`Error subscribing to peer pubsub channel ${thisPeerId}:`, err)
          this.adapters.log.statusLog(1, `Warning: Could not subscribe to peer channel ${thisPeerId}: ${err.message}`)
        }
  ...
  updatePeerConnectionInfo (inObj = {}) {
    try {
      ...
      if (connectionInfo.length === 0) {
        this.adapters.log.statusLog(
          1,
          `updatePeerConnectionInfo(): Not connected to peer ${thisPeer}`
        )
        return false
      }
