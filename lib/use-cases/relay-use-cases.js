    ...
    try {
      ...
      for (let i = 0; i < knownRelays.length; i++) {
        ...
        if (connectedPeer.length) {
          ...
          this.adapters.log.statusLog(3, `Already connected to Circuit Relay ${thisRelay.multiaddr}`)
        } else {
  ...
  async addRelay (ipfsId, thisNode) {
    try {
      ...
      for (let i = 0; i < filteredMultiaddrs.length; i++) {
        ...
        // 10/19/23 CT: Comment out this section as I don't think it's needed
        // with Helia. This code would analize a successful connection and
        // ensure the connection did not go through another circuit relay.
        // Right now with Helia, I'm happy to just make a connection. I can
        // come back and analize the type of connection later.
        ...
        //   const thisRelay = peers.filter(x => x === ipfsId)
        //   console.log('thisRelay: ', thisRelay)
        //
        //   const relayPeerId = peerIdFromString(thisRelay[0])
        //   const relayData = await this.adapters.ipfs.ipfs.libp2p.peerStore.get(relayPeerId)
        //   console.log('relayData: ', relayData)
        //
        //   multiaddr = thisRelay[0].addr
