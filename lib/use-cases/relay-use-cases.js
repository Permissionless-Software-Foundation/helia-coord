...
class RelayUseCases {
  constructor (localConfig = {}) {
    ...
    this.injectDeps = this.injectDeps.bind(this)
    ...
    // State
    this.thisNode = {} // placeholder
    this.peerUseCases = {} // placeholder
  }

  // This function is called by the peer use-case library to inject dependencies
  // needed by this libary.
  injectDeps (inObj = {}) {
    const { thisNode, peerUseCases } = inObj

    this.thisNode = thisNode
    this.peerUseCases = peerUseCases
  }
    ...
    try {
      ...
      for (let i = 0; i < knownRelays.length; i++) {
        ...
        if (connectedPeer.length && thisRelay.multiaddr.includes('/tcp/')) {
          ...
          // This node is connected to the relay, but the multiaddr needs to be fixed.
  ...
  sortRelays (relayData) {
    try {
      ...
      for (let i = 0; i < relayData.length; i++) {
        ...
        thisRelay.latencyScore = this.config.MAX_LATENCY
        ...
        if (thisRelay.isBootstrap) {
          ...
          if (isNaN(thisRelay.latencyScore)) thisRelay.latencyScore = this.config.MAX_LATENCY
        }
    ...
    try {
      ...
      for (let i = 0; i < thisNode.relayData.length; i++) {
        ...
        let peerData = thisNode.peerData.filter(x => {
          // 2/25/25 CT: Removing these lines as I don't think this code path has executed in a long time.
          // if (!x.data) {
        ...
        if (!thisRelay.connected) {
          thisRelay.metrics.aboutLatency.push(this.config.MAX_LATENCY)
          continue
        }
        ...
        // console.log(`measureRelays() startTime: ${startTime.toLocaleString()}`)
        const testResult = await this.peerUseCases.queryAbout(
          thisRelay.ipfsId,
          thisNode
        )
        ...
        // console.log(`measureRelays() endTime: ${endTime.toLocaleString()}`)

        let diffTime = this.config.MAX_LATENCY
