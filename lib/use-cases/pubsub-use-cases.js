...
class PubsubUseCase {
  ...
  async initializePubsub (thisNode) {
    try {
      ...
      // to the network.
      console.log(`INIT: subscribing to coordination pubsub channel: ${DEFAULT_COORDINATION_ROOM}`)
      ...
      // relay the messages.
      console.log(`INIT: subscribing to CoinJoin pubsub channel: ${BCH_COINJOIN_ROOM}`)
      await this.adapters.pubsub.subscribeToPubsubChannel(
        BCH_COINJOIN_ROOM,
